...
export class EventCenter {
	 ...
	 * @memberof EventCenter
	 */
	async runEventQueue(name: string, config: UserConfig,initCtx:object={}) {
		const eventList = this.eventMap[name];
		if (!eventList) {
			console.error(`未查询到该事件${name}`);
			return;
		}
		let eventFnMap=[],eventFnArgsMap=[];
		...
		let eventQuene = new EventQuene(name, eventFnMap,eventFnArgsMap,config,initCtx);
		return eventQuene.runGenerator();
	}
}
